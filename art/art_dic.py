# -*- coding: utf-8 -*-
"""1-line art data."""
art_dic = {
    "fish1": "><(((('>",
    "fish2": "><>",
    "fish3": "`·.¸¸ ><((((º>.·´¯`·><((((º>    ",
    "fish4": "><>     ><>",
    "fish5": "<><",
    "fish6": "<`)))><",
    "house": "__̴ı̴̴̡̡̡ ̡͌l̡̡̡ ̡͌l̡*̡̡ ̴̡ı̴̴̡ ̡̡͡|̲̲̲͡͡͡ ̲▫̲͡ ̲̲̲͡͡π̲̲͡͡ ̲̲͡▫̲̲͡͡ ̲|̡̡̡ ̡ ̴̡ı̴̡̡ ̡͌l̡̡̡̡.___",
    "care crowd": "(-(-_(-_-)_-)-)",
    "monster": "٩(̾●̮̮̃̾•̃̾)۶",
    "monster2": "٩(- ̮̮̃-̃)۶",
    "boombox1": "♫♪.ılılıll|̲̅̅●̲̅̅|̲̅̅=̲̅̅|̲̅̅●̲̅̅|llılılı.♫♪",
    "butterfly": "Ƹ̵̡Ӝ̵̨̄Ʒ",
    "finger1": "╭∩╮(Ο_Ο)╭∩╮",
    "pistols1": r"¯¯̿̿¯̿̿'̿̿̿̿̿̿̿'̿̿'̿̿̿̿̿'̿̿̿)͇̿̿)̿̿̿̿ '̿̿̿̿̿̿\̵͇̿̿\=(•̪̀●́)=o/̵͇̿̿/'̿̿ ̿ ̿̿",
    "finger2": "┌∩┐(◣_◢)┌∩┐",
    "heart1": "»-(¯`·.·´¯)->",
    "mouse1": '----{,_,">',
    "mouse2": " .       ~~(__^·>",
    "mouse3": "<·^__)~~          .",
    "mouse4": '—-{,_,”><",_,}----',
    "mouse5": "<:3 )~~~~",
    "mouse6": "<^__)~",
    "mouse7": "~(__^>",
    "worm": r"_/\__/\__0>",
    "koala": "@( * O * )@",
    "monkey": "@('_')@",
    "waves": "°º¤ø,¸¸,ø¤º°`°º¤ø,¸,ø¤°º¤ø,¸¸,ø¤º°`°º¤ø,¸",
    "glasses": "-@-@-",
    "rose1": "--------{---(@",
    "rose2": "@}}>-----",
    "rose3": "@-->--->---",
    "rose4": "@}~}~~~",
    "rose5": "@-}--",
    "rose6": "@)}---^-----",
    "rose7": "@->-->---",
    "star in my eyes": "<*_*>",
    "looking face": "ô¿ô",
    "sleeping": "(-.-)Zzz...",
    "sleeping baby": "[{-_-}] ZZZzz zz z...",
    "love you": [
        "»-(¯`·.·´¯)->",
        "<-(¯`·.·´¯)-«"],
    "message1": [
        "(¯`·._.·(¯`·._.· ",
        " ·._.·´¯)·._.·´¯)"],
    "message2": [
        ",.-~*´¨¯¨`*·~-.¸-(",
        "-,.-~*´¨¯¨`*·~-.¸"],
    "pistols2": r"̿' ̿'\̵͇̿̿\з=(◕_◕)=ε/̵͇̿̿/'̿'̿ ̿",
    "pistols3": r" ̿̿ ̿̿ ̿’̿̿’̿\̵͇̿̿\з=( ͡ °_̯͡° )=ε/̵͇̿̿/’̿̿’̿ ̿ ̿̿ ̿̿",
                "knife1": ")xxxxx[;;;;;;;;;>",
                "knife2": ")xxx[::::::::::>",
                "coffee1": "c[_]",
                "coffee2": "l_D",
                "coffee3": "l_P",
                "coffee4": "l_B",
                "robot1": "d[ o_0 ]b",
                "pig1": "^(*(oo)*)^",
                "needle1": "┣▇▇▇═─",
                "needle2": "|==|iiii|>----- ",
                "cat1": "=^..^=",
                "fish swim": "¸.·´¯`·.´¯`·.¸¸.·´¯`·.¸><(((º>",
                "sword1": "(===||:::::::::::::::>",
                "rock on1": r"\,,/(^_^)\,,/",
                "rock on2": r"\m/(-_-)\m/",
                "caterpillar": r",/\,/\,/\,/\,/\,/\,o",
                "sword2": "▬▬ι═══════ﺤ    -═══════ι▬▬",
                "professor": '"""⌐(ಠ۾ಠ)¬"""',
                "sad1": "ε(´סּ︵סּ`)з",
                "sword3": "ס₪₪₪₪§|(Ξ≥≤≥≤≥≤ΞΞΞΞΞΞΞΞΞΞ>",
                "airplane1": " ‛¯¯٭٭¯¯(▫▫)¯¯٭٭¯¯’",
                "cassette1": "|[●▪▪●]|",
                "cassette2": "[¯ↂ■■ↂ¯]",
                "car race": "∙،°.  ˘Ô≈ôﺣ   » » »",
                "robot2": " c[○┬●]כ ",
                "happy": r" ۜ\(סּںסּَ` )/ۜ",
                "love in my eye1": "(♥_♥)",
                "love in my eye2": "(｡❤◡❤｡)",
                "love in my eye3": "(❤◡❤)",
                "cat2": "龴ↀ◡ↀ龴",
                "cat3": "^.--.^",
                "face": "•|龴◡龴|•",
                "big nose": "˚∆˚",
                "big eyes": "⺌∅‿∅⺌",
                "woman": "▓⚗_⚗▓",
                "dog": "ˁ˚ᴥ˚ˀ",
                "stars": "✌⊂(✰‿✰)つ✌",
                "hairstyle": "⨌⨀_⨀⨌",
                "eyes": "℃ↂ_ↂↃ",
                "cat face": "⦿⽘⦿",
                "cute cat": "^⨀ᴥ⨀^",
                "nose": r"\˚ㄥ˚\\",
                "pirate": "✌(◕‿-)✌",
                "happy2": "⎦˚◡˚⎣",
                "human": "•͡˘㇁•͡˘",
                "awesome": "<:3 )~~~",
                "dagger": "cxxx|;:;:;:;:;:;:;:;>",
                "umadbro": r"¯\_(ツ)_/¯ ",
                "table flip": "(╯°□°）╯︵ ┻━┻",
                "mis mujeres": "(-(-_(-_-)_-)-)",
                "linqan": ":Q___",
                "sad2": "(✖╭╮✖)",
                "sad3": "(◑﹏◐)",
                "sad4": "(◕_◕)",
                "eye closed": " (╯_╰)",
                "inlove": "(✿ ♥‿♥) ",
                "cry": " (╯︵╰,)",
                "100$": "[̲̅$̲̅(̲̅ιοο̲̅)̲̅$̲̅]",
                "help": "٩(͡๏̯͡๏)۶",
                "birds": "~(‾▿‾)~",
                "pig2": "༼☉ɷ⊙༽",
                "bullshit": "|3ᵕᶦᶦᶳᶣᶨᶵ",
                "gun1": "︻╦╤─",
                "gun2": "︻デ═一",
                "gun3": "╦̵̵̿╤─ ҉ ~ •",
                "bird": " (⌒▽⌒)﻿",
                "sword4": " |O/////[{:;:;:;:;:;:;:;:;>",
                "elephant": "°j°m",
                "headphone1": "d[-_-]b",
                "headphone2": "d(-_-)b",
                "headphone3": "(W)",
                "up": r"(◔/‿\◔)",
                "airplane2": "✈",
                "real face": "( ͡° ͜ʖ ͡°)﻿",
                "angry face": "(⋟﹏⋞)",
                "heart2": "♡",
                "boobs": "(.)(.)",
                "sperm": "~~o",
                "ping pong": "( •_•)O*¯`·.¸.·´¯`°Q(•_• )",
                "chess": "♞▀▄▀▄♝▀▄ ",
                "scissors": "✄",
                "chair": "╦╣",
                "zombie": "'º_º'",
                "hybrix": "ʕʘ̅͜ʘ̅ʔ",
                "bear": "ʕ•ᴥ•ʔ",
                "cheer": "  ^(¤o¤)^",
                "hug me": "(っ◕‿◕)っ",
                "arrow1": "»»---------------------►",
                "arrow2": "XXX-------->",
                "rak": "/⦿L⦿\\",
                "nope": "t(-_-t)",
    "panda": "ヽ(￣(ｴ)￣)ﾉ",
    "guitar": "c====(=#O| ) ~~ ♬·¯·♩¸¸♪·¯·♫¸ ",
    "rocket": "∙∙∙∙∙·▫▫ᵒᴼᵒ▫ₒₒ▫ᵒᴼᵒ▫ₒₒ▫ᵒᴼᵒ☼)===>",
    "ghost": "‹’’›(Ͼ˳Ͽ)‹’’›",
    "hal": "@_'-'",
    "cthulhu": "^(;,;)^",
    "sat": "'(◣_◢)'",
    "what?": "ة_ة",
    "king": "-_-",
    "tron": r"(\/)(;,,;)(\/)",
    "homer": "(_8(|)",
    "fox": "-^^,--,~",
    "singing": "d(^o^)b¸¸♬·¯·♩¸¸♪·¯·♫¸¸",
    "atish": "(| - _ - |)",
    "zable": "ಠ_ರೃ",
    "trumpet": "-=iii=<()",
    "teddy": "ˁ(⦿ᴥ⦿)ˀ",
    "dice": "[: :]",
    "bee": "¸.·´¯`·¸¸.·´¯`·.¸.-<\\^}0=:",
    "ukulele": "{ o }==(::) ",
    "perky": "( ๏ Y ๏ )",
    "snail1": "'-'_@_",
    "snail2": r"'\Q___",
    "mango": ") _ _ __/°°¬",
    "decorate": "▂▃▅▇█▓▒░۩۞۩        ۩۞۩░▒▓█▇▅▃▂",
    "kirby": "(つ -‘ _ ‘- )つ",
    "bunny": r"(\_/) ",
    "kiss": "(o'3'o)",
    "sorreh bro": "(◢_◣)",
    "owlkin": "(ᾢȍˬȍ)ᾢ ļ ļ ļ ļ ļ",
    "yolo": "Yᵒᵘ Oᶰˡʸ Lᶤᵛᵉ Oᶰᶜᵉ",
    "formula1 car": r"\ō͡≡o˞̶",
    "dummy": "<-|-'_'-|->",
    "rope": "╚(▲_▲)╝",
    "rare": "┌ಠ_ಠ)┌∩┐ ᶠᶸᶜᵏ♥ᵧₒᵤ",
    "chess pieces": "♚ ♛ ♜ ♝ ♞ ♟ ♔ ♕ ♖ ♗ ♘ ♙",
    "sparkling heart": "-`ღ´-",
    "weather": "☼ ☀ ☁ ☂ ☃ ☄ ☾ ☽ ❄ ☇ ☈ ⊙ ☉ ℃ ℉ ° ❅ ✺ ϟ",
    "stars2": "⋆ ✢ ✣ ✤ ✥ ✦ ✧ ✩ ✪ ✫ ✬ ✭ ✮ ✯ ✰ ★",
    "upsidedown": "( ͜。 ͡ʖ ͜。)",
    "nathan": "♪└(￣◇￣)┐♪└(￣◇￣)┐♪└(￣◇￣)┐♪",
    "cat smile": "≧◔◡◔≦﻿",
    "old lady boobs": r"|\o/\o/|",
    "glasses2": "ᒡ◯ᵔ◯ᒢ",
    "religious": "☪ ✡ † ☨ ✞ ✝ ☥ ☦ ☓ ♁ ☩",
    "sniperstars": "✯╾━╤デ╦︻✯",
    "kokain": r" ̿ ̿' ̿'\̵͇̿̿\з=(•̪●)=ε/̵͇̿̿/'̿''̿ ̿",
    "bagel": "nln >_< nln",
    "crying1": "Ỏ̷͖͈̞̩͎̻̫̫̜͉̠̫͕̭̭̫̫̹̗̹͈̼̠̖͍͚̥͈̮̼͕̠̤̯̻̥̬̗̼̳̤̳̬̪̹͚̞̼̠͕̼̠̦͚̫͔̯̹͉͉̘͎͕̼̣̝͙̱̟̹̩̟̳̦̭͉̮̖̭̣̣̞̙̗̜̺̭̻̥͚͙̝̦̲̱͉͖͉̰̦͎̫̣̼͎͍̠̮͓̹̹͉̤̰̗̙͕͇͔̱͕̭͈̳̗̭͔̘̖̺̮̜̠͖̘͓̳͕̟̠̱̫̤͓͔̘̰̲͙͍͇̙͎̣̼̗̖͙̯͉̠̟͈͍͕̪͓̝̩̦̖̹̼̠̘̮͚̟͉̺̜͍͓̯̳̱̻͕̣̳͉̻̭̭̱͍̪̩̭̺͕̺̼̥̪͖̦̟͎̻̰_Ỏ̷͖͈̞̩͎̻̫̫̜͉̠̫͕̭̭̫̫̹̗̹͈̼̠̖͍͚̥͈̮̼͕̠̤̯̻̥̬̗̼̳̤̳̬̪̹͚̞̼̠͕̼̠̦͚̫͔̯̹͉͉̘͎͕̼̣̝͙̱̟̹̩̟̳̦̭͉̮̖̭̣̣̞̙̗̜̺̭̻̥͚͙̝̦̲̱͉͖͉̰̦͎̫̣̼͎͍̠̮͓̹̹͉̤̰̗̙͕͇͔̱͕̭͈̳̗̭͔̘̖̺̮̜̠͖̘͓̳͕̟̠̱̫̤͓͔̘̰̲͙͍͇̙͎̣̼̗̖͙̯͉̠̟͈͍͕̪͓̝̩̦̖̹̼̠̘̮͚̟͉̺̜͍͓̯̳̱̻͕̣̳͉̻̭̭̱͍̪̩̭̺͕̺̼̥̪͖̦̟͎̻̰ ",
    "angry2": "( ͠° ͟ʖ ͡°)﻿",
    "3": "ᕙ༼ ,,ԾܫԾ,, ༽ᕗ",
    "5": "ᕙ༼ ,,இܫஇ,, ༽ᕗ",
    "fuck you": "nlm (-_-) mln ",
    "head shot": "->~∑≥_≤)",
    "metal": r"\m/_(>_<)_\m/",
    "killer": "(⌐■_■)--︻╦╤─ - - - (╥﹏╥) ",
    "fu": "(ಠ_ಠ)┌∩┐",
    "ankush": "︻デ┳═ー*----*",
    "sex symbol": "◢♂◣◥♀◤◢♂◣◥♀◤",
    "barbell": "▐━━━━━▌",
    "worm2": "~",
    "sniper rifle": "︻デ┳═ー",
    "being draged": "╰(◣﹏◢)╯",
    "possessed": "<>_<>",
    "jokeranonimous": "╭∩╮ (òÓ,) ╭∩╮",
    "jokeranonimous2": "╭∩╮(ô¿ô)╭∩╮",
    "epic gun": "︻┳デ═— ",
    "love": "ⓛⓞⓥⓔ",
    "eric": ">--) ) ) )*>",
    "puls": r"––•–√\/––√\/––•––",
    "sky free": "ѧѦ ѧ  ︵͡︵  ̢ ̱ ̧̱ι̵̱̊ι̶̨̱ ̶̱   ︵ Ѧѧ  ︵͡ ︵   ѧ Ѧ    ̵̗̊o̵̖  ︵  ѦѦ ѧ ",
    "smug bastard": "(‾⌣‾)",
    "tie-fighter": "|—O—|",
    "kyubey": "／人 ⌒ ‿‿ ⌒ 人＼",
    "kyubey2": "／人 ◕‿‿◕ 人＼",
    "love2": " ~♡ⓛⓞⓥⓔ♡~",
    "dancee": "♪┏(°.°)┛┗(°.°)┓┗(°.°)┛┏(°.°)┓ ♪",
    "mtmtika": ":o + :p = 69",
    "ak-47": "︻┳デ═—",
    "eaten apple": "[===]-'",
    "huhu": "█▬█ █▄█ █▬█ █▄█",
    "faydre": "(U) [^_^] (U) ",
    "domino": "[: :|:::]",
    "honeycute": "❤◦.¸¸.  ◦✿",
    "superman": "-^mOm^-",
    "nose2": "|'L'|",
    "hell yeah": r"(òÓ,)_\,,/",
    "roke": r"_\m/",
    "crayons": "((̲̅ ̲̅(̲̅C̲̅r̲̅a̲̅y̲̅o̲̅l̲̲̅̅a̲̅( ̲̅((>",
    "stars in my eyes": "<*_*>",
    "fish invasion": "›(̠̄:̠̄c  ›(̠̄:̠̄c  (¦Ҝ  (¦Ҝ  ҉  -  -  -   ¦̺͆¦  ▪▌",
    "bender": " ¦̵̱ ̵̱ ̵̱ ̵̱ ̵̱(̢ ̡͇̅└͇̅┘͇̅ (▤8כ−◦",
    "musical": ["¸¸♬·¯·♩¸¸♪·¯·♫¸¸", "¸¸♬·¯·♩¸¸♪·¯·♫¸¸"],
    "sunny day": "☁ ▅▒░☼‿☼░▒▅ ☁",
    "happy birthday 1": "ዞᏜ℘℘Ꮍ ℬℹℛʈዞᗬᏜᎽ",
    "line brack": "●▬▬▬▬๑۩۩๑▬▬▬▬▬●",
    "med": "ب_ب",
    "melp1": "(<>..<>)",
    "melp2": "(<(<>(<>.(<>..<>).<>)<>)>)",
    "happy square": "【ツ】",
    "snowman1": "☃",
    "i kill you": r" ̿ ̿̿'̿̿\̵͇̿̿\=(•̪●)=/̵͇̿̿/'̿̿ ̿ ̿",
    "happy3": "㋡",
    "happy4": "^_^",
    "happy5": "[^_^]",
    "jaymz": " (•̪●)==ε/̵͇̿​̿/’̿’̿ ̿ ̿̿    `(•.°)~",
    "text decoration": ["(¯`·._.··¸.-~*´¨¯¨`*·~-.,-(_", "_)-,"
                        ".-~*´¨¯¨`*·~-.¸··._.·´¯)"],
    "long rose": "---------------------{{---<((@)",
    "kirby dance": "<(''<)  <( ' ' )>  (> '')>",
    "death star defense team": "|-o-| (-o-) |-o-|",
    "chainsword": " |O/////[{:;:;:;:;:;:;:;:;>",
    "boobies": "(. )( .)",
    "dancing people": r"‎(/.__.)/   \(.__.\)",
    "dance": r'''(>'-')> <('_'<) ^('_')\- \m/(-_-)\m/ <( '-')> \_( .")> <(._.)-`''',
    "pictou": r"|\_______(#*#)_______/|",
    "polar bear": "ˁ˚ᴥ˚ˀ",
    "go away bear": "╭∩╮ʕ•ᴥ•ʔ╭∩╮",
    "charly": "+:)",
    "train": r"/˳˳_˳˳\!˳˳X˳˳!(˳˳_˳˳)[˳˳_˳˳]",
    "spot": "(  . Y .  )",
    "westbound fish": "< )))) ><    ",
    "telephone": "ε(๏_๏)з】",
    "9/11 truth": "✈__✈ █ █ ▄ ",
    "spear": ">>-;;;------;;-->",
    "srs face": "(ಠ_ಠ)",
    "this is areku": "d(^o^)b",
    "robot boy": "◖(◣☩◢)◗",
    "med man": "(⋗_⋖)",
    "angry": " ლ(ಠ益ಠ)ლ",
    "zoidberg": r"(\/)(Ö,,,,Ö)(\/) ",
    "eastbound fish": "><((((>",
    "kilroy was here": '" Ü "',
    "gtalk fit": "(•̪̀●́)=ε/̵͇̿̿/'̿̿ ̿ ̿̿  N --------{---(@",
    "thanks": r"\(^-^)/",
    "dalek": r" ̵̄/͇̐\ ",
    "sean the sheep": "<('--')>",
    "party time": "┏(-_-)┛┗(-_-﻿ )┓┗(-_-)┛┏(-_-)┓",
    "kablewee": r" 	̿' ̿'\̵͇̿̿\з=( ͡ °_̯͡° )=ε/̵͇̿̿/'̿'̿ ̿ ",
    "i dont care": "╭∩╮（︶︿︶）╭∩╮",
    "slenderman": "ϟƖΣNd€RMαN",
    "john lennon": "((ºjº))",
    "peace yo!": "(‾⌣‾)♉",
    "punch": "O=('-'Q)",
    "russian boobs": "[.][.]",
    "fuck off": "t(-.-t)",
    "man tears": "ಥ_ಥ",
    "robber": r" -╤╗_(◙◙)_╔╤-   -  -  -  \o/ \o/ \o/",
    "facepalm": "(>ლ)",
    "yo": "__o000o__(o)(o)__o000o__",
    "badass": "(⌐■_■)--︻╦╤─ - - -",
    "cigarette1": "(̅_̅_̅_̅(̅_̅_̅_̅_̅_̅_̅_̅_̅̅_̅()ڪے",
    "cigarette2": "(____((____________()~~~",
    "cigarette3": "()___)____________)",
    "oar": "===========(8888)",
    "sword5": "<%%%%|==========>",
    "sword6": "o()xxxx[{::::::::::::::::::::::::::::::::::>",
    "sword7": "o==[]::::::::::::::::>",
    "sword8": "▬▬ι═══════> ",
    "sword9": " <═══════ι▬▬",
    "sword10": "O==I======>",
    "car": "`o##o>",
    "hacksaw": "[|^^^^^^^",
    "canoe": r".,.,\______/,..,.,",
    "spider1": "//O\\",
    "machinegun": ",==,--",
    "roadblock": "X+X+X+X+X",
    "aliens": "(<>..<>)",
    "marge simpson": "()()():|",
    "homer simpson": "=(:o)",
    "bat1": "^O^",
    "bat2": " ^v^ ",
    "superman logo": "/s\\",
    "dna sample": "~",
    "mail box": "|M|/",
    "ufo1": ".-=-.",
    "ufo2": ".-=o=-.",
    "spider2": r"/\oo/\\",
    "spider3": r"///\oo/\\\\\\",
    "american money1": "[($)]",
    "american money2": "[̲̅$̲̅(̲̅1̲̅)̲̅$̲̅]",
    "american money3": "[̲̅$̲̅(̲̅5̲̅)̲̅$̲̅]",
    "american money4": "[̲̅$̲̅(̲̅ιοο̲̅)̲̅$̲̅]",
    "american money5": "[̲̅$̲̅(̲̅2οο̲̅)̲̅$̲̅]",
    "british money": "[£::]",
    "teepee": "/|\\",
    "heart3": "<3",
    "angel1": "^i^",
    "shark": "~~~~~~^~~~~~",
    "regular ass": "(_!_)",
    "fat ass": "(__!__)",
    "kiss my ass": "(_x_)",
    "devil": "]:->",
    "shrug": r"¯\_(ツ)_/¯",
    "flex": "ᕙ(⇀‸↼‶)ᕗ",
    "this guy": "(☞ﾟ∀ﾟ)☞",
    "why": "ლ( `Д’ ლ)",
    "meditation": "‿( ́ ̵ _-`)‿",
    "kitty1": "=^..^=",
    "pac man": "ᗧ···ᗣ···ᗣ··",
    "emo": "(///_ ;)",
    "shark attack": r"~~~~~~\o/~~~~~/\~~~~~",
    "shocked1": "(∩╹□╹∩)",
    "monocle": "(╭ರ_•́)",
    "piggy": "(∩◕(oo)◕∩)",
    "camera": '[◉"]',
    "what??": "(Ͼ˳Ͽ)..!!!",
    "neo": "(⌐■_■)--︻╦╤─ - - -",
    "lenny": "( ͡° ͜ʖ ͡°)",
    "snowing": "✲´*。.❄¨¯`*✲。❄。*。",
    "dick": "8====D",
    "wizard": "(∩ ͡° ͜ʖ ͡°)⊃━☆ﾟ. *",
    "wat": "ಠ_ಠ",
    "chu": "(´ε｀ )",
    "butt": "(‿|‿)",
    "sophie": '<XX""XX>',
    "joy": "n_n",
    "bautista": "(╯°_°）╯︵ ━━━",
    "smooth": "(づ ￣ ³￣)づ ⓈⓂⓄⓄⓉⒽ ",
    "fuckall": " 	╭∩╮（︶︿︶）╭∩╮ ",
    "penis": "8===D",
    "wat-wat": "Σ(‘◉⌓◉’)",
    "fail": "o(╥﹏╥)o",
    "dead eyes": "¿ⓧ_ⓧﮌ",
    "satan": "↑_(ΦwΦ;)Ψ",
    "bomb": "!!(　’ ‘)ﾉﾉ⌒●~*",
    "sleepy coffee": "( -_-)旦~",
    "high five": "( ⌒o⌒)人(⌒-⌒ )v",
    "wtf dude?": "＼(◑д◐)＞∠(◑д◐)",
    "fungry": "Σ_(꒪ཀ꒪」∠)_",
    "derp": "ヘ（。□°）ヘ",
    "drowning": "人人人ヾ( ;×o×)〃 人人人",
    "almost cared": "╰╏ ◉ 〜 ◉ ╏╯",
    "yessir": "∠(･`_´･ )",
    "coffee now": "{zzz}°°°( -_-)>c[_]",
    "round cat": "~(^._.)",
    "squee": "ヾ(◎o◎,,；)ﾉ",
    "round bird": ",(u°)>",
    "hoxom": "h(o x o )m",
    "squid": "くコ:彡",
    "man spider": r"/╲/\༼ *ಠ 益 ಠ* ༽/\╱\\",
    "spell cast": "╰( ⁰ ਊ ⁰ )━☆ﾟ.*･｡ﾟ",
    "jazz musician": "ヽ(⌐■_■)ノ♪♬",
    "myancat": "mmmyyyyy<⦿⽘⦿>aaaannn",
    "gimme": "༼ つ ◕_◕ ༽つ",
    "crab": r"(\|) ._. (|/)",
    "playing in snow": "(╯^□^)╯︵ ❄☃❄ ",
    "sunglasses1": "(•_•)>⌐■-■ (⌐■_■)",
    "mini penis": "=D",
    "victory": "V(-.o)V",
    "don fuller": "╭∩╮(Ο_Ο)╭∩╮",
    "dgaf": "┌∩┐(◣ _ ◢)┌∩┐",
    "ryans dick": "8======D",
    "eds dick": "8=D",
    "point": "(☞ﾟヮﾟ)☞",
    "afraid": "(　ﾟ Дﾟ)",
    "laughing": "(＾▽＾)",
    "energy": " つ ◕_◕ ༽つ  つ ◕_◕ ༽つ ",
    "drunkenness": "ヽ（´ー｀）┌",
    "error": "(╯°□°)╯︵ ɹoɹɹƎ",
    "penis2": "○○)=======o)",
    "exchange": "(╯°□°）╯︵ ǝƃuɐɥɔxǝ",
    "singing2": "♪└(￣◇￣)┐♪",
    "acid": "⊂(◉‿◉)つ",
    "arrowhead": "⤜(ⱺ ʖ̯ⱺ)⤏",
    "awkward": "•͡˘㇁•͡˘",
    "because": "∵",
    "blackeye": "0__#",
    "catlenny": "( ͡° ᴥ ͡°)",
    "dab": "ヽ( •_)ᕗ",
    "damnyou": "(ᕗ ͠° ਊ ͠° )ᕗ",
    "depressed": "(︶︹︶)",
    "dunno": r"¯\(°_o)/¯",
    "eeriemob": "(-(-_-(-_(-_(-_-)_-)-_-)_-)_-)-)",
    "envelope": "✉",
    "fart": "(ˆ⺫ˆ๑)<3",
    "finn": "| (• ◡•)|",
    "frown": "(ღ˘⌣˘ღ)",
    "glitter": "(*・‿・)ノ⌒*:･ﾟ✧",
    "gotit": "(☞ﾟ∀ﾟ)☞",
    "hello": "(ʘ‿ʘ)╯",
    "loading1": "█▒▒▒▒▒▒▒▒▒",
    "loading2": "███▒▒▒▒▒▒▒",
    "loading3": "█████▒▒▒▒▒",
    "loading4": "███████▒▒▒",
    "loading5": "█████████▒",
    "loading6": "██████████",
    "meep": r"\(°^°)/",
    "cup1": "(▓",
    "cup2": r"\̅_̅/̷̚ʾ",
    "barcode1": "█║▌│ █│║▌ ║││█║▌ │║║█║ │║║█║",
    "barcode2": "║█║▌║█║▌│║▌║▌█║",
    "band aid": "(̲̅:̲̅:̲̅:̲̅[̲̅ ̲̅]̲̅:̲̅:̲̅:̲̅ )",
    "electrocardiogram1": "√v^√v^√v^√v^√♥",
    "electrocardiogram2": r"v^v^v^v^√\/♥",
    "electrocardiogram3": r"/\/\/\/\/\/\/\/\/\/\/\\v^♥",
    "electrocardiogram4": "√v^√v^♥√v^√v^√",
    "smile": ":-)",
    "toungue out1": ":-Þ",
    "toungue out2": ":-P",
    "bad hair1": "=:-)",
    "bad hair2": "=:-(",
    "woops": ":-*",
    "screaming": ":-@",
    "full mouth": ":-I",
    "cussing": ":-#",
    "ready to cry": ":-}",
    "crying2": ":~(",
    "druling1": ":-...",
    "druling2": ":-P~~~",
    "big kiss": ":-X",
    "french kiss": ":-XÞ",
    "wink": ";-)",
    "big smile": ":-D",
    "smirk": ":-,",
    "confused1": ":-/",
    "confused2": ":-\\",
    "pursing lips": ':-"',
    "shocked2": ":-O",
    "really sad": ":-C",
    "baseball fan": "q:o)",
    "angel2": "O:-)",
    "really mad": ">:-I",
    "licking lips": ":-9",
    "buck teeth": ":-B",
    "surprised1": "=:-o",
    "alien": "::)",
    "sunglasses2": "B-)",
    "don king": "==8-)",
    "devilish smile": ">:)",
    "devilish grin": ">:-D",
    "clowning": "*:o)",
    "bugs bunny": "E=B",
    "kitty2": "=^. .^=",
    "fish skeleton1": ">-}-}-}->",
    "fish skeleton2": ">++('>",
    "snowman2": '{ }( : ^ )( """" )( )',
    "marshmallows": "-()_)--()_)---",
    "nerd": "::(",
    "surprised2": "(O_o)",
    "boombox2": "♫♪ |̲̅̅●̲̅̅|̲̅̅=̲̅̅|̲̅̅●̲̅̅| ♫♪",
    "equalizer": "▇ ▅ █ ▅ ▇ ▂ ▃ ▁ ▁ ▅ ▃ ▅ ▅ ▄ ▅ ▇",
    "playing cards": "[♥]]] [♦]]] [♣]]] [♠]]]",
    "playing cards hearts": "[♥]]]",
    "playing cards diamonds": "[♦]]]",
    "playing cards spades": "[♠]]]",
    "playing cards clubs": "[♣]]]",
    "spade bold": "♠",
    "heart bold": "♥",
    "diamond bold": "♦",
    "club bold": "♣",
    "spade regular": "♤",
    "heart regular": "♡",
    "diamond regular": "♢",
    "club regular": "♧",
    "loch ness monster": "_mmmP",
    "sheep": "Â°lÂ°(,,,,);",
    "pie fight": "---=======[}",
    "concerned": "(@_@)",
    "carpet roll": "@__",
    "infinity": "(X)",
    "computer mouse": "[E}",
    "volcano1": '/"\\',
    "volcano2": "/W\\",
    "volcano3": "/V\\",
    "squigle with spirals": r"6\9",
    "palm tree": "'T`",
    "crotch shot": r"\*/",
    "vagina": "(:)",
    "stealth fighter": "-^-",
    "tent1": "//\\",
    "tent2": "/\\\\",
    "power lines": "TTT",
    "tree stump": 'J"l',
    "hammer": "#==",
    "fork": "---=",
    "pipe": r"====\_/",
    "dead guy": "'==xx\\0",
    "dead girl": r"'==>x\9",
    "dead child": "'-=,o",
    "dude glasses1": "@[O],[O]",
    "dude glasses2": "@(o),(o)"

}
